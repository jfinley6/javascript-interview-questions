...
function connectToRedis(env) {
  ...
  if (config.redis) {
    ...
    return new Promise((resolve, reject) => {
      if (!auth) {
        reject(
          new Error("Connection to Redis failed - no valid auth token found")
        );
      }
    ...
    return Promise.reject(
      new Error("Connection to Redis failed - No valid Redis config found")
    );
  }
}
